...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    const DA = firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        const userdocreference = usersreference.doc(user.uid);
  ...
  useEffect(() => {
    // get all posts from the Firestore "reports" collection
    const postsreference = firebase.firestore().collection("reports");
    postsreference.get().then((Qarg) => {
      const posts = [];
      Qarg.forEach((doc) => {
        const data = doc.data();
  ...
  const handleformsubmit = (event) => {
    ...
    const postId = firebase.firestore().collection("reports").doc().id;
    // create new Firestore document with post data
    firebase.firestore().collection("reports").doc(postId).set({
      postId,
      authorId,
      title,
      content,
  ...
  const handledelete = (postId) => {
    ...
    // delete the document with the given ID from the Firestore "reports" collection
    firebase.firestore().collection("reports").doc(postId).delete().then(() => {
          ...
          {posts.map((post, index) => {
            ...
            return (
              <div key={index} className="post">
                <h2 className="post-heading">Date added: {timestamp.toString()}</h2>
